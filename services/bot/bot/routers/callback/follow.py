...
def _build_unfollow_podcast_reply_markup(podcast_id: PydanticObjectId, podcast_link: str) -> InlineKeyboardMarkup:
    kbd = InlineKeyboardBuilder()

    kbd.button(text="Unfollow", callback_data=UnfollowCallbackData(podcast_id=podcast_id))
...
async def handle_follow_query(query: CallbackQuery, user: User, callback_data: FollowCallbackData):
    try:
    ...
    try:
        ...
        await query.answer(f"🚫 You already follow {podcast.db_object.meta.title}", show_alert=True)
        return

    markup = _build_unfollow_podcast_reply_markup(podcast.db_object.id, podcast.db_object.meta.link)
