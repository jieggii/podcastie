...
def _build_follow_podcast_reply_markup(
...
async def handle_unfollow_callback(
    ...
    try:
        ...
        await query.answer(
            f"🚫 You do not follow {podcast.db_object.meta.title}", show_alert=True
        )
        return

    markup = _build_follow_podcast_reply_markup(
