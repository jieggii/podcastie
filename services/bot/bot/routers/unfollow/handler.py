...
from bot.callback_data.subscriptions import (
    SubscriptionsCTACallbackData as SubscriptionsListCallbackData,
)
from bot.callback_data.unfollow import (
    ReturnTo,
    UnfollowCallbackData,
    UnfollowCTACallbackData,
    UnfollowPromptCallbackData,
)
...
def _match_return_to(
    return_to: ReturnTo,
) -> UnfollowCTACallbackData | SubscriptionsListCallbackData:
    match return_to:
...
async def handle_unfollow_view(
    query: CallbackQuery, bot: Bot, user: User, callback_data: UnfollowCallbackData
):
    try:
    ...
    try:
        ...
        await query.message.answer(
            "Error: you do not follow this podcast."
        )  # todo: better error handling
        return
    ...
    await query.answer(
        f"You have successfully unsubscribed from {bold(podcast.db_object.meta.title)}"
    )
    await query.message.edit_text(
        f"You have successfully unsubscribed from {podcast.db_object.meta.title}",
        reply_markup=markup,
    )


@router.callback_query(UnfollowPromptCallbackData.filter())
async def handle_confirmation_view(
    query: CallbackQuery,
    bot: Bot,
    user: User,
    callback_data: UnfollowPromptCallbackData,
):
    try:
        podcast = await Podcast.from_object_id(callback_data.podcast_id)
    except PodcastNotFoundError:
    ...
    kbd.button(
        text="Yes",
        callback_data=UnfollowCallbackData(
            podcast_id=podcast.db_object.id, return_to=callback_data.return_to
        ),
    )
    ...
    await query.message.edit_text(
        f"Are you sure you want to unfollow {bold(podcast.db_object.meta.title)}?",
        reply_markup=markup,
    )
