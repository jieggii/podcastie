...
from bot.callback_data.open_view import OpenFindPodcastView, OpenImportView
...
async def send_cta(
    bot: Bot, user: User, *, chat_id: int, edit_message_id: int | None = None
):
    ...
    if not subscriptions:
        ...
        kbd.button(
            text="Find a podcast",
            callback_data=OpenFindPodcastView(edit_current_message=True),
        )
        kbd.button(
            text="Import subscriptions",
            callback_data=OpenImportView(edit_current_message=True),
        )
        markup = kbd.as_markup()

        if edit_message_id:
            await bot.edit_message_text(
                text, chat_id=chat_id, message_id=edit_message_id, reply_markup=markup
            )
        else:
    ...
    for podcast in subscriptions:
        kbd.button(
            ...
            callback_data=UnfollowPromptCallbackData(
                podcast_id=podcast.db_object.id, return_to=ReturnTo.UNFOLLOW_LIST
            ),
        )
    kbd.adjust(2)
    markup = kbd.as_markup()
