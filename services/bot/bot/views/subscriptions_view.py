...
from bot.aiogram_view.util import answer_entrypoint_event, answer_callback_query_entrypoint_event
from bot.callback_data.entrypoints import PodcastViewEntrypointCallbackData, SubscriptionsViewEntrypointCallbackData
...
class SubscriptionsView(View):
    async def handle_entrypoint(self, event: CallbackQuery, data: dict[str, typing.Any] | None = None) -> None:
        ...
        if callback_data.unfollow_podcast_id:
            try:
                podcast = await Podcast.from_object_id(callback_data.unfollow_podcast_id)
                try:
                    await user.unfollow_podcast(podcast)
                    await event.answer(f"Successfully unfollowed {podcast.db_object.meta.title}")
                except UserDoesNotFollowPodcastError:
                    await event.answer("Failed to unfollow podcast as you do not follow it anymore.")
            except PodcastNotFoundError:
                await event.answer("Failed to unfollow podcast as it does not longer exist.")
