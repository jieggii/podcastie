...
def new_bot(bot_token: str, bot_api_host: str, bot_api_port: int) -> Bot:
    session = AiohttpSession(
        api=TelegramAPIServer.from_base(f"http://{bot_api_host}:{bot_api_port}")
    )
...
async def main() -> None:
    ...
    episode_broadcaster = EpisodeBroadcaster(episodes_queue, bot=new_bot(env.TelegramBot.TOKEN, env.TelegramBot.API_HOST, env.TelegramBot.API_PORT), interval=1)
